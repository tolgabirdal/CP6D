...
import argparse
import numpy as np
from Keypoint.ALIKED.nets.aliked import ALIKED
...
def model_selection(model_name: str,top_k=1000, device='cuda'):
    if model_name == 'aliked-t16':
        model = ALIKED(model_name=model_name, device=device, top_k=top_k, scores_th=0.2, n_limit=5000)
    elif model_name == 'aliked-n16':
        model = ALIKED(model_name=model_name, device=device, top_k=top_k, scores_th=0.2, n_limit=5000)
    elif model_name == 'aliked-n16rot':
        model = ALIKED(model_name=model_name, device=device, top_k=top_k, scores_th=0.2, n_limit=5000)
    elif model_name == 'aliked-n32':
        model = ALIKED(model_name=model_name, device=device, top_k=top_k, scores_th=0.2, n_limit=5000)
    else:
        raise ValueError(f"Model {model_name} not available!")
    return model

def keypoint(img_rgb, model):
    img_rgb = img_rgb
    model = model
